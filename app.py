...
def respiratory_page():
    ...
    if form.validate_on_submit():
        ...
        if form.breath.data == 'cannotbreath':
            output_dict['id_0'] = 1
        else:
            pass


        if form.breath.data == 'id_11':
            output_dict['id_11'] = 1
...
def conversation_page():
    ...
    if form.validate_on_submit():
        ...
        if id_34:
            output_dict['id_34'] = 1
            if session['output_data']['age'] and timeoftalk < 3:
                output_dict['id_41'] = 1
            else:
                pass
        else:
...
def output_page():
    ...
    model_path = 'C:\\CBDweb\\multioutput_rf_model.pkl'
    loaded_model = joblib.load(model_path)
    predictions = loaded_model.predict(X_test_df)
    predictions = loaded_model.predict(X_test_df)
    code = predictions[0,0]
    colour = predictions[0,1]
    # Prepare data and call process function
    data = [session['output_data']['name'], session['output_data']['age'],code, colour, X_test_df]
    
    process_output_data(data)
    send_to_DB(data)
    return render_template('output.html', prediction=[code,colour])
...
def send_to_DB(data):
    try:
        ...
        # Extract DataFrame values as a list of lists
        df_values = data[4].values.tolist()  # DataFrame is the 5th element

        # Prepare the final list: [timestamp, name, age, code, colour, *DataFrame rows]
        flat_data = [[timestamp, data[0], data[1], data[2], data[3]] + row for row in df_values]

        # Prepare the request body for the Sheets API
        body = {
            'values': flat_data
        }
